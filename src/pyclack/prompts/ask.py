...
class Ask(PromptBase):
    ...
    def handle_active(self, key: Optional[str]) -> bool:
        ...
        if self.placeholder and len(input_buffer) <= 0:
            ...
            frame_builder.add_lines(*build_wrapped_styled_input_lines(placeholder_text, prefix_active, 0, True))
        else:
            input_buffer_text: Text = Text(input_buffer, theme.text)
            frame_builder.add_lines(*build_wrapped_styled_input_lines(input_buffer_text, prefix_active, input_index, True))
    ...
    def handle_error(self, key: Optional[str]) -> bool:
        ...
        if self.placeholder and len(input_buffer) <= 0:
            placeholder_text: Text = Text(self.placeholder, theme.muted)
            frame_builder.add_lines(*build_wrapped_styled_input_lines(placeholder_text, prefix_error, 0, True))
        else:
            input_buffer_text: Text = Text(input_buffer, theme.text)
            frame_builder.add_lines(*build_wrapped_styled_input_lines(input_buffer_text, prefix_error, input_index, True))

        validation_error_text: Text = Text(self.validate(input_buffer), theme.error) # self.validate must be defined if we are in the error state, and it must return something
